...
class PlayMarker {
    ...
    var root = NSXMLElement(name: "div")
    var node : NSXMLElement
    ...
    private func scan(token: String) -> Bool {
        return scanner.scanString(token, intoString: nil)
    }

    private func scanTo(token: String) -> String {
        var string: NSString?
        if scanner.scanUpToString(token, intoString: &string) {
        } else {
    ...
    func generateContents() {
        ...
        let contentDoc = NSXMLDocument(rootElement: playground)
        contentDoc.standalone = true
        ...
        output += [("contents.xcplayground", contentDoc.XMLStringWithOptions(Int(NSXMLNodePrettyPrint)))]
    ...
    func createDocument(content: NSXMLElement) -> NSXMLDocument {
        let rootElement = element(nil, "html", ("lang", "en"))

        let doc = NSXMLDocument(rootElement: rootElement)
        doc.standalone = true
        doc.documentContentKind = .XHTMLKind
