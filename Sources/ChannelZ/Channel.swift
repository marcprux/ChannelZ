...
public extension ChannelType {
    ...
    @inlinable func lift<Pulse2>(_ receptor: @escaping (@escaping (Pulse2) -> Void) -> ((Pulse) -> Void)) -> Channel<Source, Pulse2> {
    ...
    @inlinable func map<U>(_ transform: @escaping (Pulse) -> U) -> Channel<Source, U> {
...
public extension ChannelType {
    ...
    @inlinable func merge<C2: ChannelType>(_ with: C2) -> Channel<(Source, C2.Source), Pulse> where C2.Pulse == Pulse {
    ...
    @inlinable func either<C2: ChannelType>(_ other: C2) -> Channel<(Source, C2.Source), Choose2<Pulse, C2.Pulse>> {
    ...
    @inlinable func combine<C2: ChannelType>(_ other: C2) -> Channel<(Self.Source, C2.Source), (Pulse, C2.Pulse)> {
    ...
    @inlinable func zip<C2: ChannelType>(_ with: C2, capacity: (Int, Int) = (Int.max, Int.max)) -> Channel<(Self.Source, C2.Source), (Pulse, C2.Pulse)> {
...
public extension ChannelType {
    ...
    @inlinable func flatMap<S2, U>(_ transform: @escaping (Pulse) -> Channel<S2, U>) -> Channel<Source, U> {
...
public extension ChannelType {
    ...
    @inlinable func mergeMap<U>(_ transform: @escaping (Pulse) -> Channel<Pulse, U>) -> Channel<Source, Choose2<Pulse, U>> {
...
public extension Channel {
    ...
    @inlinable func concat(_ with: Channel<Source, Pulse>) -> Channel<[Source], (Source, Pulse)> {
...
public extension ChannelType {
    ...
    @inlinable func affect<T>(_ seed: @escaping @autoclosure () -> T, affector: @escaping (T, Pulse) -> T) -> Channel<Source, (store: T, pulse: Pulse)> {
    ...
    @inlinable func reduce<T>(_ initial: T, combine: @escaping (T, Pulse) -> T) -> Channel<Source, T> {
    ...
    @inlinable func enumerate() -> Channel<Source, (index: Int, pulse: Pulse)> {
    ...
    @inlinable func partition<U>(_ initial: U, includePartitions: Bool = true, isPartition: @escaping (U, Pulse) -> Bool, combine: @escaping (U, Pulse) -> U) -> Channel<Source, U> {
    ...
    @inlinable func accumulate(includePartitions: Bool = true, predicate: @escaping ([Pulse], Pulse) -> Bool) -> Channel<Source, [Pulse]> {
    ...
    ///
    /// - Returns: A stateful Channel that maintains an accumulation of pulses
    @inlinable func split(_ maxSplit: Int = Int.max, allowEmptySlices: Bool = false, isSeparator: @escaping (Pulse) -> Bool) -> Channel<Source, [Pulse]> {
    ...
    @inlinable func buffer(_ limit: Int) -> Channel<Source, [Pulse]> {
    ...
    @inlinable func dropFirst(_ n: Int = 1) -> Channel<Source, Pulse> {
    ...
    ///
    /// - Returns: A stateful Channel that drops the first `count` pulses.
    @inlinable func prefix(_ maxLength: Int) -> Channel<Source, Pulse> {
